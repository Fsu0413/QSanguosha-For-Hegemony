    ...
    try {
        if (card_use.card->getRealCard() == card) {
            ...
            if (card->isKindOf("DelayedTrick") && card->isVirtualCard() && card->subcardsLength() == 1) {
                ...
                Q_ASSERT(trick != NULL);
                Card *wrapped = Sanguosha->getCard(card->getSubcards().first());
            ...
            if (!card_use.card->isVirtualCard()) {
                Card *wrapped = Sanguosha->getCard(card_use.card->getEffectiveId());
                if (wrapped->isModified())
...
{
    for (int i = 0; i < move.card_ids.size(); i++) {
        Card *card = qobject_cast<Card *>(getCard(move.card_ids[i]));
        if (card->isModified()) {
...
void Room::filterCards(ServerPlayer *player, QList<const Card *> cards, bool refilter)
{
    if (refilter) {
        for (int i = 0; i < cards.size(); i++) {
            Card *card = qobject_cast<Card *>(getCard(cards[i]->getId()));
            if (card->isModified()) {
...
void Room::askForGuanxing(ServerPlayer *zhuge, const QList<int> &cards, QSgsEnum::RearrangeCardDirection guanxing_type)
{
